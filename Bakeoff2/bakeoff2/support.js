...
{ 
  ...
  //start_button.position(width/2 - start_button.size().width/2, height/3.4 - start_button.size().height/2);

  instructions_button = createButton('See Example');
  instructions_button.mouseReleased(drawInstructions);
  instructions_button.position(width/2 - instructions_button.size().width/2, height/2 - instructions_button.size().height/2);
}
...
function drawInstructions(){
  ...
  if (validID() && validSize()) {
    ...
    instructions_button.remove();

    fullscreen(!fullscreen());
    resizeCanvas(displayWidth, displayHeight);

    instructions = createImg('./instructions.png');
    instructions.size(displayWidth, displayHeight);
    instructions.position(0, 0);
    
    // 3. Start button
    start_button = createButton('START');
    start_button.mouseReleased(startTest);
    start_button.position(displayWidth - 120 , displayHeight*0.85 );
    start_button.size(100,50);
    start_button.style('background-color', 'rgb(209, 193, 92)');
